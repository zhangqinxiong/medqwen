...
% 张钦雄  2112505018
% Compile with: xelatex -> xelatex (twice for references)
...
\begin{abstract}
Large language models show promise for clinical applications, but adapting them to medicine typically requires a lot of compute that many labs cannot afford. We ask whether lightweight, parameter-efficient fine-tuning can bridge this gap between general-purpose capabilities and specialized medical knowledge under realistic resource constraints.
...
In the next section we cover related work on medical LLMs and PEFT. Section~\ref{sec:method} describes our model setup, LoRA configuration, and data pipeline. Experiments and results follow in Section~\ref{sec:experiments}, with discussion and conclusion in Section~\ref{sec:discussion} and Section~\ref{sec:conclusion}.
...
\begin{quote}
\texttt{你是一个专业的临床医生，请根据患者的问题}\\
\texttt{提供专业、准确、安全的医疗建议。}
\end{quote}
...
On data volume, 5K samples gives a perplexity of 6.85, dropping to 5.73 at 10K and 5.18 at 20K. The gain from 10K to 20K (0.55 PPL) is similar to the gain from 5K to 10K (0.59 PPL), so more data would probably help but with diminishing returns.
